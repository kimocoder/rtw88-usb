...
#define RTW_USB_HANDLER_CHECK_LOOPS	2
...
static void rtw_usb_config_rxagg(struct rtw_dev *rtwdev)
{
	...
	dma_usb_agg = rtw_usb_read8(rtwdev, REG_RXDMA_AGG_PG_TH + 3);
	agg_enable = rtw_usb_read8(rtwdev, REG_TXDMA_PQ_MAP);

	/* RX AGG MODE USB */
	dma_usb_agg &= ~BIT(7);
	agg_enable |= BIT_RXDMA_AGG_EN;

	if (rtw_usb_read8(rtwdev, REG_SYS_CFG2 + 3) == 0x20) {
	...
	val32 = rtw_usb_read32(rtwdev, REG_RXDMA_AGG_PG_TH);
	rtw_usb_write32(rtwdev, REG_RXDMA_AGG_PG_TH, val32 | BIT_EN_PRE_CALC);

	rtw_usb_write8(rtwdev, REG_TXDMA_PQ_MAP, agg_enable);
	rtw_usb_write8(rtwdev, REG_RXDMA_AGG_PG_TH + 3, dma_usb_agg);
	rtw_usb_write16(rtwdev, REG_RXDMA_AGG_PG_TH,
...
static void rtw_usb_switch_mode(struct rtw_dev *rtwdev)
{
	...
	usb_status = rtw_usb_read32(rtwdev, REG_PAD_CTRL2);
...
static struct sk_buff *rtw_usb_tx_agg_skb(struct rtw_usb *rtwusb,
					  ...
					  struct rtw_usb_txcb_t *txcb)
{
	...
	skb_head = dev_alloc_skb(RTW_USB_MAX_XMITBUF_SZ + RTW_USB_RECVBUFF_ALIGN_SZ);
	if (!skb_head)
		return NULL;

	skb_iter = skb;
	len = ALIGN(skb_iter->len, RTW_USB_RECVBUFF_ALIGN_SZ);
	if (skb_tailroom(skb_head) < len)
		goto dealloc_skb_head;
	...
	while (skb_iter) {
		...
		max_len = skb_tailroom(skb_head) - RTW_USB_RECVBUFF_ALIGN_SZ;
					    ...
					    struct rtw_usb_txcb_t *txcb)
{
	...
	skb_head = rtw_usb_tx_agg_skb(rtwusb, list, skb, txcb);
	return skb_head;
}
...
static void rtw_usb_rx_handler(struct work_struct *work)
{
	...
	for (limit = 0; limit < RTW_USB_HANDLER_CHECK_LOOPS; limit++) {
		skb_head = skb_dequeue(&rtwusb->rx_queue);
		if (!skb_head)
			break;
		...
		while (skb_head->len > 0) {
			...
			if (pkt_stat.is_c2h) {
				...
				//rtw_info(rtwdev, "RX AGG skb size:%d(offset:%d, len:%d, len:%d\n",
				//	 size, pkt_offset, pkt_stat.pkt_len, skb->len);
